...
public class ChatServer implements ServerSocketThreadListener, SocketThreadListener {
    ...
    @Override
    public void onSocketStop(SocketThread thread) {putLog("Client disconnected"); }
    ...
    public void onReceiveString(SocketThread thread, Socket socket, String msg) {
        //clients.forEach(c -> c.sendMessage(thread.getName()  + " echo: " + msg));
        if (clients.size() != 0) {
            for (int i=0; i<clients.size(); i++){
                clients.get(i).sendMessage("Для Всех: " + msg);
            }
        }
    }
